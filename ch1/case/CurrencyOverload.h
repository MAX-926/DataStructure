...
class Currency
{
    public:
        ...
        Currency operator+(const Currency<signType>&)const;
        Currency& operator+=(const Currency<signType>& x){amount += x.amount; return *this;}
...
template<class signType>
Currency<signType> Currency<signType>::operator+(const Currency<signType>& x)const
{
    Currency<signType> y;
...
template<class signType>
void Currency<signType>::output(ostream& out)const
{// 输出调用对象的值
...
//重载 <<
template<class signType>
ostream& operator<<(ostream& os, const Currency<signType>& c){ c.output(os); return os;}
