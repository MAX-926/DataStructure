...
int binarySearch(int num[], int value, int k, int m)
{
    int middle = (m+k)/2;
    if(m == k && num[m] != value) return -1;
    if(num[middle] == value)
        return middle;
    else if(num[middle] < value)
        binarySearch(num, value, middle + 1, m);
    else
        binarySearch(num, value, k, middle - 1);
}
...
int main()
{
    ...
    cout << binarySearch(num, 10, 0, len - 1);
}
